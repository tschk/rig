...
/// Bindings for a built path/git native lib on a Nim host (`passL` link hints).
pub fn write_nim_path_native(
    out: &Path,
    name: &str,
    dep: &Dependency,
    ...
    if exports.is_empty() {
        body.push_str("# No simple C prototypes discovered; declare importc procs as needed.\n");
    } else {
        body.push_str(&surface::emit_nim_procs(&exports));
        body.push('\n');
    }
